...
class PulseDetectorProcessor:
    def __init__(self, config: DetectorConfig):
        ...
        self.ai_validator = LEDAIVerifier(history_size=10)
    ...
    def recv(self, frame):
        ...
        color_masks = build_color_masks(
            hsv,
            led_color_mode=self.config.led_color_mode,
        )
        ...
        if target is not None and ai_result["is_valid_led"]:
            raw_score = score_full * 1.30
            self.status = f"{self.last_color}"
        elif target is not None and not ai_result["is_valid_led"]:
            raw_score = score_full * 0.50
            self.status = f"{self.last_color} DUVIDOSO"
        else:
            raw_score = score_full * 0.15
            self.status = "SEM LED"

        self.instant_buffer.append(raw_score)
        instant_score = max(self.instant_buffer) if self.instant_buffer else raw_score

        self.buffer.append(raw_score)
        smooth_score = float(sum(self.buffer) / len(self.buffer))

        # mistura resposta rápida com estabilidade
        if self.config.fast_pulse_mode:
            score = (instant_score * 0.70) + (smooth_score * 0.30)
        else:
            score = smooth_score
