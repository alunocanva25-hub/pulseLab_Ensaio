...
class PulseDetectorProcessor:
    def __init__(self, config: DetectorConfig):
        ...
        self.contador = ContadorPulso(limiar_on=float(config.limiar_on), limiar_off=float(config.limiar_off), debounce_s=float(config.debounce_ms) / 1000.0)
    ...
    def recv(self, frame):
        ...
        score = float(sum(self.buffer) / len(self.buffer))
        self.score = score
        ...
        if self.config.show_overlay:
            ...
            cv2.putText(img, f"{self.status} | P:{self.pulsos}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 255, 0), 2)
