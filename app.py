...
def list_users_df() -> pd.DataFrame:
    ...
    if not rows:
        ...
        return pd.DataFrame(
            ...
            columns=["id", "username", "full_name", "role", "is_active", "created_at", "updated_at"]
    ...
    if st.session_state.app_logo_bytes:
        ...
        st.markdown(
            ...
            <div class="topbar-wrap">
                ...
                <img src="data:image/png;base64,{b64}" style="height:48px;border-radius:8px;">

                <h1 class="logo-title">PulseLab v6</h1>
...
with st.sidebar:
    ...
    st.session_state.camera_habilitada = st.toggle(

        "Habilitar câmera",

        value=st.session_state.camera_habilitada,

    )



    st.session_state.tempo_automatico = st.toggle(

        "Tempo automático",

        value=st.session_state.tempo_automatico,

    )



    st.session_state.alvo_pulsos_auto = st.number_input(
        ...
        value=int(st.session_state.alvo_pulsos_auto),
    ...
    st.session_state.tempo_minimo_seg = st.number_input(
        ...
        value=int(st.session_state.tempo_minimo_seg),
    ...
    st.session_state.tempo_maximo_seg = st.number_input(
        ...
        value=int(st.session_state.tempo_maximo_seg),
    ...
    st.session_state.mostrar_qrcode = st.toggle(

        "Mostrar QR Code",

        value=st.session_state.mostrar_qrcode,
...
if selected == "Ensaio":
    ...
    rows = CONN.execute(
        ...
        "SELECT ts, actor_username, event_type, details FROM audit_log ORDER BY id DESC LIMIT 200"
